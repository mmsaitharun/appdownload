...
//			response.setUrl("https://appdownloaddee8964f1.us2.hana.ondemand.com/AppDownload/download?fileType="+fileType);
		...
		if(!AppUtils.isEmpty(fileType)) {
			response = RestUtil.callRest("https://a65dee8964f1.us2.hana.ondemand.com/WORKBOX/GetFromAPP_FILE.xsjs?fileType='"+fileType+"'");
		}
		return response;
	}
...
//			Connection con = DriverManager.getConnection("jdbc:sap://localhost:30115/WORKBOX", "WORKBOX", "Incture1234567891013");
			Statement stmt = con.createStatement();
//			Map<String, String> versionMap = new HashMap<String, String>();
			ResultSet versionResult = stmt.executeQuery("SELECT FILE_URL AS FILE_URL, MAX(FILE_VERSION) AS VERSION FROM APP_FILE WHERE FILE_TYPE = '"+fileType.toUpperCase()+"' GROUP BY(FILE_URL)");
			while(versionResult.next()) {
